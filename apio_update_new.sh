...
if [ "$gitStatus" -ne 0 ]; then
  ...
  log "riavvio app.js e attendo 60 secondi"
  cd "../${folderName}"
  forever start -s -c "node --expose_gc" app.js
  sleep 60

  log "memorizzo i PID dei processi"
  newServices=$(node -e '
  "use strict";
  let config = undefined;
  try {
  ...
  newServices=${newServices%]}
  # create an array
  eval newServices=(${newServices})

  declare -A servicesMap
  for service in "${newServices[@]}"
  do
    servicesMap[${service}]=$(ps aux | grep ${service} | grep 'node --expose_gc' | xargs)
  done

  servicesMap['app.js']=$(ps aux | grep app.js | grep 'node --expose_gc' | xargs)

  sleep 60
  log "controllo che i PID dei processi siano gli stessi dopo aver aspettato ulteriori 60 secondi"
  for service in "${!servicesMap[@]}"
  do
    pid=$(ps aux | grep ${service} | grep 'node --expose_gc' | xargs)

    if [ "$pid" -ne "${servicesMap[$service]}" ]; then
      log "il processo ha un PID diverso rispetto a prima, installazione fallita. ripristino le cartelle"
